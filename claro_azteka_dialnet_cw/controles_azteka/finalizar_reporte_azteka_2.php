...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_azteka_registrado   = $_POST['id_azteka_registrado'];
    ...
    try {
        ...
        $sql = "INSERT INTO tb_azteka_finalizacion 
                (id_azteka_registrado, fecha_hora_finalizado, horas_totales, horas_real_dano, tipo_de_dano, parada_reloj, hora_parada_inicio, hora_parada_fin, horas_parada, observaciones_final) 
                VALUES 
                (:id_azteka_registrado, :fecha_hora_finalizado, :horas_totales, :horas_real_dano, :tipo_dano, :parada_reloj, :hora_parada_inicio, :hora_parada_fin, :horas_parada, :observaciones_final)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':id_azteka_registrado'   => $id_azteka_registrado,
        ...
        $sqlUpdate = "UPDATE tb_azteka SET estado = 'Finalizado' WHERE id_azteka_registrado = :id";
        $stmtUpdate = $pdo->prepare($sqlUpdate);
        $stmtUpdate->execute([':id' => $id_azteka_registrado]);

        $pdo->commit();

        bitacora($pdo, $_SESSION['id_usuario'], 'ACTUALIZAR', 'tb_azteka', $id_azteka_registrado, "Reporte finalizado ID: $id_azteka_registrado");
        $id_finalizado = $pdo->lastInsertId();
        bitacora($pdo, $_SESSION['id_usuario'], 'CREAR', 'tb_azteka_finalizacion', $id_finalizado, "Finalización creada para reporte ID: $id_azteka_registrado");
        ...
        <script>
            Swal.fire({
                ...
                window.location = '../vistas_azteka/lista_azteka_2.php';
            });
        </script>
        </body>
        </html>";
