...
$id_claro_registrado = isset($_GET['id_claro_registrado']) ? (int) $_GET['id_claro_registrado'] : 0;

// Buscar datos del Reporte
$sqlReporte = "SELECT * FROM tb_claro WHERE id_claro_registrado = :id_claro_registrado LIMIT 1";
$queryReporte = $pdo->prepare($sqlReporte);
$queryReporte->execute([':id_claro_registrado' => $id_claro_registrado]);
...
if (!$reporte) {
    ...
    <script>
        Swal.fire({
            ...
            text: 'El reporte con ID " . $id_claro_registrado . " no existe o fue eliminado.',
            confirmButtonText: 'Volver'
        }).then(() => {
            window.location = 'lista_claro_2.php';
        });
    </script>";
    exit;
}
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="../controles/finalizar_reporte_claro_2.php" enctype="multipart/form-data">
                                <?= csrf_field() ?>
                                <input type="hidden" name="id_claro_registrado" value="<?= $reporte['id_claro_registrado'] ?>">
                                ...
                                <div class="text-end">
                                    ...
                                    <a href="lista_claro_2.php" class="btn btn-secondary">Cancelar</a>
                                </div>
                            </form>
                        </div> 
                    </div> 
