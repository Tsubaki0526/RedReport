...
$id_dialnet_registrado = isset($_GET['id_dialnet_registrado']) ? (int) $_GET['id_dialnet_registrado'] : 0;

// Buscar datos del Reporte
$sqlReporte = "SELECT * FROM tb_dialnet WHERE id_dialnet_registrado = :id_dialnet_registrado LIMIT 1";
$queryReporte = $pdo->prepare($sqlReporte);
$queryReporte->execute([':id_dialnet_registrado' => $id_dialnet_registrado]);
...
if (!$reporte) {
    ...
    <script>
        Swal.fire({
            ...
            text: 'El reporte con ID " . $id_dialnet_registrado . " no existe o fue eliminado.',
            confirmButtonText: 'Volver'
        }).then(() => {
            window.location = 'lista_dialnet_2.php';
        });
    </script>";
    exit;
}
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="../controles_dialnet/finalizar_reporte_dialnet_2.php" enctype="multipart/form-data">
                                <?= csrf_field() ?>
                                <input type="hidden" name="id_dialnet_registrado" value="<?= $reporte['id_dialnet_registrado'] ?>">
                                ...
                                <div class="text-end">
                                    ...
                                    <a href="lista_dialnet_2.php" class="btn btn-secondary">Cancelar</a>
                                </div>
                            </form>
                        </div> 
                    </div> 
