...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_dialnet_registrado   = $_POST['id_dialnet_registrado'];
    ...
    try {
        ...
        $sql = "INSERT INTO tb_dialnet_finalizacion 
                (id_dialnet_registrado, fecha_hora_finalizado, horas_totales, horas_real_dano, tipo_de_dano, parada_reloj, hora_parada_inicio, hora_parada_fin, horas_parada, observaciones_final) 
                VALUES 
                (:id_dialnet_registrado, :fecha_hora_finalizado, :horas_totales, :horas_real_dano, :tipo_dano, :parada_reloj, :hora_parada_inicio, :hora_parada_fin, :horas_parada, :observaciones_final)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':id_dialnet_registrado'   => $id_dialnet_registrado,
        ...
        $sqlUpdate = "UPDATE tb_dialnet SET estado = 'Finalizado' WHERE id_dialnet_registrado = :id";
        $stmtUpdate = $pdo->prepare($sqlUpdate);
        $stmtUpdate->execute([':id' => $id_dialnet_registrado]);

        $pdo->commit();

        bitacora($pdo, $_SESSION['id_usuario'], 'ACTUALIZAR', 'tb_dialnet', $id_dialnet_registrado, "Reporte finalizado ID: $id_dialnet_registrado");
        $id_finalizado = $pdo->lastInsertId();
        bitacora($pdo, $_SESSION['id_usuario'], 'CREAR', 'tb_dialnet_finalizacion', $id_finalizado, "Finalización creada para reporte ID: $id_dialnet_registrado");
        ...
        <script>
            Swal.fire({
                ...
                window.location = '../vistas_dialnet/lista_dialnet_2.php';
            });
        </script>
        </body>
        </html>";
