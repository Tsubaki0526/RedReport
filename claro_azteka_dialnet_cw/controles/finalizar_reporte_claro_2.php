...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_claro_registrado   = $_POST['id_claro_registrado'];
    ...
    try {
        ...
        $sql = "INSERT INTO tb_claro_finalizacion 
                (id_claro_registrado, fecha_hora_finalizado, horas_totales, horas_real_dano, tipo_de_dano, parada_reloj, hora_parada_inicio, hora_parada_fin, horas_parada, observaciones_final) 
                VALUES 
                (:id_claro_registrado, :fecha_hora_finalizado, :horas_totales, :horas_real_dano, :tipo_dano, :parada_reloj, :hora_parada_inicio, :hora_parada_fin, :horas_parada, :observaciones_final)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':id_claro_registrado'   => $id_claro_registrado,
        ...
        $sqlUpdate = "UPDATE tb_claro SET estado = 'Finalizado' WHERE id_claro_registrado = :id";
        $stmtUpdate = $pdo->prepare($sqlUpdate);
        $stmtUpdate->execute([':id' => $id_claro_registrado]);

        $pdo->commit();

        bitacora($pdo, $_SESSION['id_usuario'], 'ACTUALIZAR', 'tb_claro', $id_claro_registrado, "Reporte finalizado ID: $id_claro_registrado");
        $id_finalizado = $pdo->lastInsertId();
        bitacora($pdo, $_SESSION['id_usuario'], 'CREAR', 'tb_claro_finalizacion', $id_finalizado, "Finalización creada para reporte ID: $id_claro_registrado");
        ...
        <script>
            Swal.fire({
                ...
                window.location = '../vistas/lista_claro_2.php';
            });
        </script>
        </body>
        </html>";
